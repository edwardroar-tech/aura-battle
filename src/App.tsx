  ...
  return (
    <div className="auth-shell">
      <div className="auth-brand">
        <div className="brand">⚡ <span>Aura farming battles</span><b>V5.176.4</b></div>
...
function VideoCard({title,videoRef,score,muted=false,bot=false}:{title:string;videoRef:RefObject<HTMLVideoElement|null>;score:number;muted?:boolean;bot?:boolean}){const [aspect,setAspect]=useState("16/9");return <div className="video-card"><h3>{title}</h3><div className="video-frame" style={{aspectRatio:aspect}}>{bot?<div className="bot-camera-placeholder"><span>🤖</span><strong>Rival IA</strong><small>Jugador ficticio</small><b>⚡ {score} Aura</b></div>:<video ref={videoRef} autoPlay playsInline muted={muted} onLoadedMetadata={e=>{const v=e.currentTarget;if(v.videoWidth&&v.videoHeight)setAspect(`${v.videoWidth}/${v.videoHeight}`)}}/>}</div><div className="score-line"><div className="aura-label"><span>⚡ NIVEL DE AURA</span><b>{score} Aura</b></div><div className="bar"><span style={{width:`${Math.min(100,score)}%`}}/></div></div></div>}

...
useEffect(()=>{
 ...
 async function inviteFriendToClan(clan:Clan, friend:Friend){
  if(!user)return
  const admin=user.uid===clan.owner||user.uid===clan.coLeader
  if(!admin||clan.members?.includes(friend.id))return
  const requestId=`${clan.id}_invite_${friend.id}`; const ref=doc(db,'clanJoinRequests',requestId)
  try{const snap=await getDoc(ref);if(snap.exists()&&snap.data().status==='pending'){setClanMsg('ℹ️ Ya existe una invitación pendiente para este jugador.');return} await setDoc(ref,{clanId:clan.id,requesterId:user.uid,requesterName:user.displayName||'Jugador',receiverId:friend.id,clanName:clan.name,kind:'invite',status:'pending',createdAt:serverTimestamp()});setClanMsg(`📨 Invitación enviada a ${friend.name}.`)}catch(error:any){console.error('Invite to clan error:',error);setClanMsg(`❌ No se pudo enviar la invitación${error?.code?` (${error.code})`:''}.`)}
 }
 async function acceptClanInvite(r:ClanJoinRequest){
  if(!user)return
  const clan=clans.find(c=>c.id===r.clanId)
  if(!clan){setClanMsg('⚠️ El clan ya no está disponible.');return}
  if(myClanId&&myClanId!==clan.id){setClanMsg('⚠️ Ya perteneces a otro clan.');return}
  try{await runTransaction(db,async tx=>{const clanRef=doc(db,'clans',clan.id);const reqRef=doc(db,'clanJoinRequests',r.id);const clanSnap=await tx.get(clanRef);const reqSnap=await tx.get(reqRef);if(!clanSnap.exists()||!reqSnap.exists())throw new Error('invite-missing');const data=clanSnap.data();const req=reqSnap.data();if(req.receiverId!==user.uid||req.kind!=='invite'||req.status!=='pending')throw new Error('not-allowed');const members=Array.isArray(data.members)?[...data.members]:[];if(!members.includes(user.uid))members.push(user.uid);tx.update(clanRef,{members});tx.update(reqRef,{status:'accepted',reviewedAt:serverTimestamp()})});setUnreadClanInvites(prev=>prev.filter(x=>x.id!==r.id));localStorage.setItem(`auraClanInviteRead:${user.uid}:${r.id}`,'1');setClanMsg(`✅ Te uniste a [${clan.tag||'CLAN'}] ${clan.name}.`)}catch(error:any){console.error('Accept clan invite error:',error);setClanMsg(`❌ No se pudo aceptar la invitación${error?.code?` (${error.code})`:''}.`)}
 }
 async function declineClanInvite(r:ClanJoinRequest){
  if(!user)return
  ...
  return (
    <div className="auth-shell">
      <div className="auth-brand">
        <div className="brand">⚡ <span>AURA BATTLE</span><b>V5.164</b></div>

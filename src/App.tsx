...
import { notifyUser, setupPushNotifications } from './lib/notifications'
...
export default function App(){
 ...
 const pushErrorText=(reason:string)=>reason==='android-webview-native-required'?'📱 Este APK necesita FCM nativo de Android. La web sí puede registrar notificaciones en Chrome, pero una WebView no sustituye la integración nativa.':reason==='permission-denied'?'🔕 El permiso de notificaciones fue rechazado. Puedes activarlo desde los ajustes de Android/navegador.':'⚠️ No se pudieron activar las notificaciones. Comprueba HTTPS y la configuración de Firebase.'
 ...
 async function enablePushNotifications(){
   ...
   else {setPushStatus('error');setAuthMsg(pushErrorText(result.reason))}
 }
...
useEffect(()=>{
 ...
 async function addFriend(f:Friend){
  ...
  try{
    ...
    void notifyUser(f.id,'Nueva solicitud de amistad',`${user.displayName||'Jugador'} quiere agregarte como amigo.`,'/friends')
  }catch(error:any){
...
useEffect(()=>{
  ...
  try{
    ...
    socket.emit('create',async(r:any)=>{
      ...
      try{
        ...
        void notifyUser(f.id,'⚔️ Invitación a batalla',`${user.displayName||'Jugador'} te invitó a una batalla.`,'/battle')
 ...
 async function requestToJoinClan(clan:Clan){
  ...
  try{
    ...
    void notifyUser(clan.owner,'🛡️ Nueva solicitud de clan',`${user.displayName||'Jugador'} quiere unirse a [${clan.tag||'CLAN'}] ${clan.name}.`,'/clans')
    if(clan.coLeader&&clan.coLeader!==clan.owner)void notifyUser(clan.coLeader,'🛡️ Nueva solicitud de clan',`${user.displayName||'Jugador'} quiere unirse a [${clan.tag||'CLAN'}] ${clan.name}.`,'/clans')
 ...
 async function inviteFriendToClan(clan:Clan, friend:Friend){
  ...
  try{
    ...
    if(snap.exists()){
      ...
      if(data.status==='pending'&&data.senderId===user.uid&&data.receiverId===friend.id){
    ...
    void notifyUser(friend.id,'🛡️ Invitación de clan',`${user.displayName||'Jugador'} te invitó al clan [${clan.tag||'CLAN'}] ${clan.name}.`,'/clans')
  ...
  return (
    <div className="auth-shell">
      <div className="auth-brand">
        <div className="brand">⚡ <span>Aura farming battles</span><b>V5.166</b></div>

...
import { addDoc, collection, deleteDoc, doc, getDoc, getDocs, limit, onSnapshot, orderBy, query, serverTimestamp, setDoc, updateDoc, arrayUnion, where, writeBatch, increment } from 'firebase/firestore'
...
type Clan = {id:string; name:string; owner:string; members:string[]; tag?:string; description?:string; emblem?:string; visibility?:'public'|'private'; createdAt?:any}
...
export default function App(){
 ...
 const [leaders,setLeaders]=useState<Friend[]>([]); const [clans,setClans]=useState<Clan[]>([]); const [clanSearch,setClanSearch]=useState(''); const [clanName,setClanName]=useState(''); const [clanTag,setClanTag]=useState(''); const [clanDescription,setClanDescription]=useState(''); const [clanEmblem,setClanEmblem]=useState('⚡'); const [clanVisibility,setClanVisibility]=useState<'public'|'private'>('public'); const [myClanId,setMyClanId]=useState<string|null>(null); const [clanMsg,setClanMsg]=useState('')
...
useEffect(()=>{ if(!user)return; setChatLoading(true); setChatMsg(''); const q=query(collection(db,'chat'),orderBy('createdAt','desc'),limit(60)); return onSnapshot(q,s=>{setChat(s.docs.map(d=>({id:d.id,...d.data()} as ChatMsg)).reverse());setChatLoading(false)},e=>{console.error('Global chat load error:',e);setChatLoading(false);setChatMsg('⚠️ No se pudo cargar el chat global.')}) },[user])
 ...
 useEffect(()=>{ if(!user){setClans([]);setMyClanId(null);return} const q=query(collection(db,'clans'),orderBy('createdAt','desc'),limit(50)); return onSnapshot(q,s=>{ const rows=s.docs.map(d=>({id:d.id,...d.data()} as Clan)); setClans(rows); const mine=rows.find(c=>Array.isArray(c.members)&&c.members.includes(user.uid)); setMyClanId(mine?.id||null) },e=>{console.error('Clans load error:',e);setClans([])}) },[user])
...
useEffect(()=>{
 ...
 async function createClan(){
  if(!user)return
  const name=clanName.trim(); const tag=clanTag.trim().toUpperCase(); const description=clanDescription.trim();
  ...
  if(tag.length<2||tag.length>6){setClanMsg('⚠️ La etiqueta debe tener entre 2 y 6 caracteres.');return}
  ...
  try{
    const batch=writeBatch(db); const clanRef=doc(collection(db,'clans')); const userRef=doc(db,'users',user.uid)
    batch.set(clanRef,{name,tag,description,emblem:clanEmblem||'⚡',visibility:clanVisibility,owner:user.uid,members:[user.uid],createdAt:serverTimestamp()})
    batch.update(userRef,{aura:increment(-100)})
    await batch.commit()
    setProfile(prev=>({...prev,aura:Math.max(0,prev.aura-100)})); setClanName('');setClanTag('');setClanDescription('');setClanEmblem('⚡');setClanVisibility('public');setClanMsg('✅ Clan creado correctamente. Gastaste 100 Aura.');
  }catch(error:any){console.error('Create clan error:',error);setClanMsg(`❌ No se pudo crear el clan${error?.code?` (${error.code})`:''}.`)}
 }
 const filteredClans=clans.filter(c=>{const term=clanSearch.trim().toLowerCase(); if(!term)return true; return `${c.name||''} ${c.tag||''}`.toLowerCase().includes(term)})
 ...
 {tab==='clans'&&<Panel title="🛡️ Clanes"><div className="clan-create"><div className="section-title">🏰 Crear mi clan</div><div className="clan-form-grid"><input placeholder="Nombre del clan" value={clanName} onChange={e=>{setClanName(e.target.value);setClanMsg('')}}/><input placeholder="Etiqueta · 2-6 letras" maxLength={6} value={clanTag} onChange={e=>{setClanTag(e.target.value);setClanMsg('')}}/><input placeholder="Emblema · ⚡" maxLength={4} value={clanEmblem} onChange={e=>{setClanEmblem(e.target.value);setClanMsg('')}}/><select value={clanVisibility} onChange={e=>setClanVisibility(e.target.value as 'public'|'private')}><option value="public">🌎 Público</option><option value="private">🔒 Privado</option></select><textarea placeholder="Descripción del clan (opcional)" maxLength={160} value={clanDescription} onChange={e=>setClanDescription(e.target.value)}/></div><div className="clan-create-foot"><small>⚡ Crear clan cuesta <strong>100 Aura</strong>. {myClanId?'Ya perteneces a un clan.':''}</small><button className="primary" onClick={()=>void createClan()} disabled={!!myClanId}>＋ Crear clan</button></div></div>{clanMsg&&<div className="notice">{clanMsg}</div>}<div className="section-title">🔎 Buscar clanes</div><div className="inline clan-search"><input placeholder="Buscar por nombre o etiqueta" value={clanSearch} onChange={e=>setClanSearch(e.target.value)}/></div><div className="section-title">Clanes de la comunidad</div><div className="home-grid">{filteredClans.length?filteredClans.map(c=><div className="clan-card" key={c.id}><div className="clan-emblem">{c.emblem||'⚡'}</div><h3>{c.name}</h3><p className="clan-tag">[{c.tag||'CLAN'}] · {c.visibility==='private'?'🔒 Privado':'🌎 Público'}</p><p>{c.description||'Clan de la comunidad AURA BATTLE.'}</p><p>👥 {c.members?.length||0} miembros</p><small>Base creada por {c.owner===user.uid?'ti':'otro jugador'}</small></div>):<div className="empty">No encontramos clanes con esa búsqueda.</div>}</div></Panel>}
  ...
  return (
    <div className="auth-shell">
      <div className="auth-brand">
        <div className="brand">⚡ <span>AURA BATTLE</span><b>V5.162</b></div>

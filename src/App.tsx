...
type BotOpponent = {id:string; name:string; aura:number; difficulty:'Fácil'|'Media'|'Difícil'; emoji:string; style:string}

const BOT_OPPONENTS:BotOpponent[]=[
 {id:'bot-farmer-01',name:'Mateo Cosecha',aura:42,difficulty:'Fácil',emoji:'🌾',style:'Movimientos tranquilos'},
 {id:'bot-farmer-02',name:'Lola Granjera',aura:55,difficulty:'Media',emoji:'🥕',style:'Ritmo equilibrado'},
 {id:'bot-farmer-03',name:'Toro del Aura',aura:68,difficulty:'Media',emoji:'🐂',style:'Ataques constantes'},
 {id:'bot-farmer-04',name:'Don Maíz',aura:78,difficulty:'Difícil',emoji:'🌽',style:'Gran presión de Aura'},
 {id:'bot-farmer-05',name:'Café Turbo',aura:88,difficulty:'Difícil',emoji:'☕',style:'Rival rápido y agresivo'},
 {id:'bot-farmer-06',name:'Rosa Semilla',aura:38,difficulty:'Fácil',emoji:'🌱',style:'Defensa paciente'},
 {id:'bot-farmer-07',name:'Pedro Trigal',aura:47,difficulty:'Fácil',emoji:'🌾',style:'Ritmo constante'},
 {id:'bot-farmer-08',name:'Marta Zanahoria',aura:51,difficulty:'Media',emoji:'🥕',style:'Ataques rápidos'},
 {id:'bot-farmer-09',name:'Bruno Cacao',aura:59,difficulty:'Media',emoji:'🍫',style:'Golpes impredecibles'},
 {id:'bot-farmer-10',name:'Nico Tomate',aura:63,difficulty:'Media',emoji:'🍅',style:'Presión moderada'},
 {id:'bot-farmer-11',name:'Valeria Cosecha',aura:67,difficulty:'Media',emoji:'🌻',style:'Movimientos variados'},
 {id:'bot-farmer-12',name:'El Campesino',aura:72,difficulty:'Difícil',emoji:'🧑‍🌾',style:'Experiencia de campo'},
 {id:'bot-farmer-13',name:'Rayo del Maizal',aura:75,difficulty:'Difícil',emoji:'⚡',style:'Ataques veloces'},
 {id:'bot-farmer-14',name:'Ana Café',aura:79,difficulty:'Difícil',emoji:'☕',style:'Ritmo intenso'},
 {id:'bot-farmer-15',name:'Tito Cosechador',aura:82,difficulty:'Difícil',emoji:'🚜',style:'Presión continua'},
 {id:'bot-farmer-16',name:'Sofía Campestre',aura:45,difficulty:'Fácil',emoji:'🌼',style:'Juego tranquilo'},
 {id:'bot-farmer-17',name:'Lucas Frutal',aura:57,difficulty:'Media',emoji:'🍎',style:'Cambios de ritmo'},
 {id:'bot-farmer-18',name:'Mango Master',aura:69,difficulty:'Media',emoji:'🥭',style:'Ataques sorpresa'},
 {id:'bot-farmer-19',name:'La Reina del Trigo',aura:86,difficulty:'Difícil',emoji:'👑',style:'Rival de élite'},
 {id:'bot-farmer-20',name:'Capitán Cosecha',aura:91,difficulty:'Difícil',emoji:'🧑‍🌾',style:'Combate avanzado'},
 {id:'bot-farmer-21',name:'Paco Papa',aura:43,difficulty:'Fácil',emoji:'🥔',style:'Defensa sencilla'},
 {id:'bot-farmer-22',name:'Julia Frijol',aura:53,difficulty:'Media',emoji:'🫘',style:'Ritmo equilibrado'},
 {id:'bot-farmer-23',name:'Rafa Caña',aura:61,difficulty:'Media',emoji:'🌿',style:'Ataques constantes'},
 {id:'bot-farmer-24',name:'Misterio del Campo',aura:77,difficulty:'Difícil',emoji:'🎭',style:'Movimientos difíciles de leer'},
 {id:'bot-farmer-25',name:'Granjero Supremo',aura:96,difficulty:'Difícil',emoji:'🏆',style:'Rival máximo'}
]
...
export default function App(){
 ...
 const [friendSearch,setFriendSearch]=useState(''); const [friends,setFriends]=useState<Friend[]>([]); const [friendResults,setFriendResults]=useState<Friend[]>([]); const [friendMsg,setFriendMsg]=useState(''); const [friendSearching,setFriendSearching]=useState(false); const [friendRequests,setFriendRequests]=useState<FriendRequest[]>([]); const [sentFriendRequests,setSentFriendRequests]=useState<FriendRequest[]>([]); const [unreadFriendRequests,setUnreadFriendRequests]=useState<FriendRequest[]>([]); const [battleInvites,setBattleInvites]=useState<BattleInvite[]>([]); const [battleSearch,setBattleSearch]=useState(''); const [battleSearchResults,setBattleSearchResults]=useState<Friend[]>([]); const [battleSearching,setBattleSearching]=useState(false); const [selectedBot,setSelectedBot]=useState<BotOpponent|null>(null); const [botBattle,setBotBattle]=useState(false); const [notificationOpen,setNotificationOpen]=useState(false); const [unreadPrivateMessages,setUnreadPrivateMessages]=useState<ChatMsg[]>([])
 ...
 useEffect(()=>{
  if(!user){setFriendRequests([]);setSentFriendRequests([]);return}
  const q=query(collection(db,'friendRequests'),where('receiverId','==',user.uid))
  const sentQ=query(collection(db,'friendRequests'),where('senderId','==',user.uid))
  const unsubIncoming=onSnapshot(q,s=>{
    const rows=s.docs.map(d=>({id:d.id,...d.data()} as FriendRequest)).filter(r=>r.senderId&&r.receiverId===user.uid&&r.status==='pending')
  ...
  },e=>{console.error('Friend requests load error:',e);setFriendRequests([]);setUnreadFriendRequests([])})
  const unsubSent=onSnapshot(sentQ,s=>{
    const rows=s.docs.map(d=>({id:d.id,...d.data()} as FriendRequest)).filter(r=>r.senderId===user.uid&&r.status==='pending')
    setSentFriendRequests(rows)
  },e=>{console.error('Sent friend requests load error:',e);setSentFriendRequests([])})
...
async function sendPushEvent(targetUid:string,event:string,refId:string){
  ...
  try{
    ...
    if(!response.ok){
      ...
      try{const body=await response.json();detail=String(body?.error||detail)}catch{}
      console.warn('Push event delivery failed:',event,targetUid,detail)
      return
    }
    const body=await response.json().catch(()=>null)
    if(body?.ok===false)console.warn('Push event delivery failed:',event,targetUid,body?.error||'push-error')
...
useEffect(()=>{ if(!user)return; setChatLoading(true); setChatMsg(''); const q=query(collection(db,'chat'),orderBy('createdAt','desc'),limit(60)); return onSnapshot(q,s=>{setChat(s.docs.map(d=>({id:d.id,...d.data()} as ChatMsg)).reverse());setChatLoading(false)},e=>{console.error('Global chat load error:',e);setChatLoading(false);setChatMsg('⚠️ No se pudo cargar el chat global.')}) },[user])
 ...
 useEffect(()=>{
  if(!user){setUnreadClanInvites([]);return}
  // Listen only to invitations addressed to this account. This works with secure
  // Firestore rules and avoids reading the whole clanInvites collection.
  const q=query(collection(db,'clanInvites'),where('receiverId','==',user.uid),limit(100))
  return onSnapshot(q,s=>{
    const rows=s.docs.map(d=>({id:d.id,...d.data()} as ClanInvite))
      .filter(r=>r.receiverId===user.uid&&r.status==='pending'&&r.kind==='invite')
    rows.sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0))
    // Toda invitación pendiente debe volver a aparecer aunque ese mismo ID haya
    // sido usado en una invitación anterior. Cancelar/reinvitar genera de nuevo
    // el documento, por lo que no usamos localStorage para ocultarlo.
    setUnreadClanInvites(rows)
...
// Batallas contra jugadores ficticios: usan el mismo contador de 5 s y ronda de 15 s,
// pero no ocupan un socket ni cuentan como jugadores online.
useEffect(()=>{
  if(!botBattle||!battleReady||battleStarted||battleCountdown!==0)return
  battleResultHandledRef.current=false
  botResultLockRef.current=false
  setBattleResult(null)
  setBattleStarted(true)
  setBattleSeconds(15)
  setOpponentAura(0)
  setRoomStatus(`🔥 ¡BATALLA CONTRA ${selectedBot?.name||'RIVAL'}!`)
},[botBattle,battleReady,battleCountdown,battleStarted,selectedBot])

useEffect(()=>{
  if(!botBattle||!battleStarted||!selectedBot)return
  const base=selectedBot.difficulty==='Fácil'?2:selectedBot.difficulty==='Media'?4:5
  const variance=selectedBot.difficulty==='Fácil'?3:selectedBot.difficulty==='Media'?4:4
  const timer=window.setInterval(()=>{
    setOpponentAura(v=>Math.min(100,v+base+Math.floor(Math.random()*variance)))
    // Si el modelo de pose no está disponible en este teléfono, la batalla contra bot
    // sigue siendo jugable: la cámara activa aporta una cantidad pequeña de Aura.
    if(!poseLandmarkerRef.current) setAura(v=>Math.min(100,v+1+Math.floor(Math.random()*3)))
  },1000)
  return()=>window.clearInterval(timer)
},[botBattle,battleStarted,selectedBot])

useEffect(()=>{
  if(!botBattle||!battleStarted||battleSeconds!==0)return
  void finishBotBattle()
},[botBattle,battleStarted,battleSeconds])
...
useEffect(()=>{
 ...
 function returnToHome(){
   ...
   battleResultHandledRef.current=false
   setRoomCode('');setRoom('');setOpponentJoined(false);setOpponentAura(0);setBotBattle(false);setSelectedBot(null);setAura(0);movementScoreRef.current=0;previousPoseRef.current=null;poseFrameCountRef.current=0;poseVisibleFrameCountRef.current=0;setRoomStatus('Listo.')
   window.history.replaceState({auraTab:'home'},'',window.location.href)
   setTab('home')
 }
 async function finishBotBattle(){
   if(!user||!botBattle||!battleStarted||botResultLockRef.current)return
   botResultLockRef.current=true
   battleResultHandledRef.current=true
   const localScore=Math.round(Math.max(0,Math.min(100,aura)))
   const rivalScore=Math.round(Math.max(0,Math.min(100,opponentAura)))
   const draw=localScore===rivalScore
   const outcome:'win'|'loss'|'draw'=draw?'draw':(localScore>rivalScore?'win':'loss')
   const delta=outcome==='win'?25:(outcome==='draw'?10:5)
   if(localStreamRef.current){localStreamRef.current.getTracks().forEach(track=>track.stop());localStreamRef.current=null}
   if(videoRef.current)videoRef.current.srcObject=null
   if(remoteVideoRef.current)remoteVideoRef.current.srcObject=null
   peerRef.current?.close();peerRef.current=null
   setCameraOn(false);setBothCamerasReady(false);setBattleStarted(false);setBattleSeconds(0);setBattleReady(false);setOpponentReady(false)
   movementScoreRef.current=0;previousPoseRef.current=null
   setBattleResult({outcome,localScore,rivalScore,delta})
   setRoomStatus(outcome==='draw'?'🤝 Batalla empatada':outcome==='win'?'🏆 Batalla ganada':'💥 Batalla perdida')
   if(user){
     try{
       const ref=doc(db,'users',user.uid); const snap=await getDoc(ref); const current=snap.exists()?snap.data():{}
       const currentAura=Number(current.aura||0), currentWins=Number(current.victorias||0), currentLosses=Number(current.derrotas||0), currentDraws=Number(current.empates||0), currentBattles=Number(current.batallas||0), currentEarned=Number(current.auraGanada||0), currentStreak=Number(current.rachaActual||0), currentBest=Number(current.mejorRacha||0)
       const nextAura=Math.max(0,currentAura+delta), nextWins=currentWins+(outcome==='win'?1:0), nextLosses=currentLosses+(outcome==='loss'?1:0), nextDraws=currentDraws+(outcome==='draw'?1:0), nextBattles=currentBattles+1, nextStreak=outcome==='win'?currentStreak+1:0, nextBest=Math.max(currentBest,nextStreak), nextEarned=currentEarned+delta, nextLevel=Math.min(100,Math.max(1,Math.floor(nextAura/100)+1))
       await setDoc(ref,{aura:nextAura,victorias:nextWins,derrotas:nextLosses,empates:nextDraws,batallas:nextBattles,auraGanada:nextEarned,rachaActual:nextStreak,mejorRacha:nextBest,level:nextLevel},{merge:true})
       setProfile({aura:nextAura,wins:nextWins,losses:nextLosses,draws:nextDraws,battles:nextBattles,auraEarned:nextEarned,currentStreak:nextStreak,bestStreak:nextBest,level:nextLevel})
     }catch(e){console.error('Bot battle result save error:',e);setRoomStatus('⚠️ La batalla terminó, pero no pudimos guardar el resultado.')}
   }
 }
 async function finishBattle(){if(!user||!battleStarted)return;if(botBattle){await finishBotBattle();return}socket.emit('finish-battle');setRoomStatus('⏳ Finalizando la batalla…')}
 ...
 async function startCamera(){
   try{
     ...
     const videoConstraints={facingMode:{ideal:'user'},aspectRatio:{ideal:9/16},width:{ideal:720},height:{ideal:1280},frameRate:{ideal:30,max:30}}
     let s:MediaStream
     ...
     // Intentar reducir el zoom digital de la cámara frontal para conseguir un encuadre más amplio.
     // Solo se aplica si el dispositivo/navegador expone control de zoom.
     try{
       ...
       console.debug('Zoom de cámara no disponible; se mantiene el encuadre del dispositivo.',zoomError)
     }
     cameraSourceRef.current=tab==='ai'?'ai':'battle'
     attachLocalStream(s)
     setCameraOn(true)
     // Las batallas contra bots son locales: no necesitan WebRTC, socket ni una
     // segunda cámara real. Esto evita que una negociación de red bloquee el
     // botón LISTO o deje la batalla esperando indefinidamente.
     if(botBattle){
       setBothCamerasReady(true)
       setOpponentReady(true)
       setRoomStatus(micAvailable?'📷 Cámara activa. El bot está listo. Puedes pulsar LISTO.':'📷 Cámara activa. El micrófono no está disponible, pero puedes continuar contra el bot.')
       return
     }
 ...
 async function startBattle(){
   ...
   // Un bot no tiene una segunda cámara física ni necesita una sala WebRTC.
   // Solo necesitamos que la cámara local esté activa.
   if(botBattle){
     if(!selectedBot){setRoomStatus('⚠️ Selecciona un rival ficticio antes de iniciar.');return}
     if(!cameraOn){setRoomStatus('📷 Activa tu cámara antes de marcarte como listo.');return}
     setBothCamerasReady(true)
   }else if(!opponentJoined||!cameraOn||!bothCamerasReady){
     setRoomStatus('🎥 Espera a que las dos cámaras estén activas antes de marcarte como listo.');return
   }
   if(!botBattle&&!poseLandmarkerRef.current){setRoomStatus('🧠 La IA de movimiento todavía está cargando. Espera unos segundos e inténtalo de nuevo.');return}
   ...
   if(botBattle){
     setOpponentReady(true)
     setBattleCountdown(5)
     setRoomStatus(`⏳ ${selectedBot?.name||'El rival'} está listo. La batalla comienza en 5 segundos…`)
     return
   }
 ...
 async function searchRivals(){
   const term=battleSearch.trim().toLowerCase().replace(/\s+/g,' ')
   setBattleSearchResults([])
   if(!user){return}
   setBattleSearching(true)
   try{
     const snap=await getDocs(collection(db,'users'))
     const results=snap.docs.filter(d=>d.id!==user.uid).map(d=>{const x=d.data() as any;const name=String(x.nombre||x.name||x.displayName||'Jugador');const lower=String(x.nombreLower||name).toLowerCase().replace(/\s+/g,' ');return{id:d.id,name,aura:Number(x.aura||0),lower}}).filter(x=>!term||x.lower.includes(term)||x.name.toLowerCase().replace(/\s+/g,' ').includes(term)).slice(0,12)
     setBattleSearchResults(results.map(({id,name,aura})=>({id,name,aura})))
   }catch(error){console.error('Rival search error:',error);setRoomStatus('⚠️ No se pudo buscar rivales reales. Puedes jugar contra los rivales ficticios mientras tanto.')}finally{setBattleSearching(false)}
 }
 async function selectBotOpponent(bot:BotOpponent){
   // Esperamos a limpiar la sala WebRTC anterior antes de marcar al bot como
   // listo. Antes había una condición de carrera: resetPeer() podía ejecutarse
   // después de setBothCamerasReady(true) y dejar la batalla contra bot bloqueada.
   await resetPeer()
   setSelectedBot(bot)
   setBotBattle(true)
   setRoomCode(`BOT-${bot.id.slice(-2)}`)
   setRoom('')
   setHost(true)
   ...
   setBattleResult(null)
   setOpponentAura(0)
   setRoomStatus(`🤖 ${bot.name} está disponible. Activa tu cámara y pulsa LISTO.`)
   navigateTab('battle')
 }
 function clearBotOpponent(){setSelectedBot(null);setBotBattle(false);setRoomCode('');setOpponentJoined(false);setBothCamerasReady(false);setOpponentReady(false);setBattleReady(false);setOpponentAura(0);setRoomStatus('Listo.')}
...
useEffect(()=>{
 ...
 async function acceptClanInvite(r:ClanInvite){
  ...
  try{
    ...
    setUnreadClanInvites(prev=>prev.filter(x=>x.id!==r.id)); setClanMsg(`✅ Te uniste a [${String(clan.tag||r.clanTag||'CLAN')}] ${String(clan.name||r.clanName||'Clan')}.`)
 ...
 async function declineClanInvite(r:ClanInvite){
  if(!user)return
  try{await updateDoc(doc(db,'clanInvites',r.id),{status:'rejected',reviewedAt:serverTimestamp()});setUnreadClanInvites(prev=>prev.filter(x=>x.id!==r.id));setClanMsg('Invitación de clan rechazada.')}catch(error:any){console.error('Decline clan invite error:',error);setClanMsg(`❌ No se pudo rechazar la invitación${error?.code?` (${error.code})`:''}.`)}
 }
 ...
 </div>:<><div className="battle-intro"><div><h2>Entra a la arena</h2><p>Busca un rival o juega contra uno de los 25 granjeros ficticios. Los bots no cuentan como jugadores online.</p></div><div className="live-dot">● LIVE</div></div><div className="rival-search-card"><div className="section-title">🔎 Buscar rival</div><div className="rival-search-row"><input maxLength={40} placeholder="Nombre del rival (opcional)" value={battleSearch} onChange={e=>setBattleSearch(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')void searchRivals()}}/><button type="button" className="primary" onClick={()=>void searchRivals()} disabled={battleSearching}>{battleSearching?'🔎 Buscando…':'🔎 Buscar'}</button></div><div className="bot-grid">{BOT_OPPONENTS.filter(b=>!battleSearch.trim()||b.name.toLowerCase().includes(battleSearch.trim().toLowerCase())).map(bot=><div className="bot-rival-card" key={bot.id}><div className="bot-avatar">{bot.emoji}</div><div className="bot-info"><strong>{bot.name}</strong><span>⚡ Aura {bot.aura} · {bot.difficulty}</span><small>{bot.style}</small></div><button type="button" className="primary" onClick={()=>void selectBotOpponent(bot)}>⚔️ Jugar</button></div>)}</div>{battleSearchResults.length>0&&<div className="real-rival-list"><h3>👥 Jugadores reales</h3>{battleSearchResults.map(r=><div className="real-rival-row" key={r.id}><span>👤 {r.name}<small>⚡ {r.aura} Aura</small></span>{friends.some(f=>f.id===r.id)?<button type="button" onClick={()=>void inviteBattleWithFriend(r)}>⚔️ Invitar</button>:<small>Jugador real · usa una sala/código</small>}</div>)}</div>}</div><div className="room-card">{!roomCode?<div className="room-actions"><button className="primary" onClick={createRoom}>{t.create}</button><span>o</span><input maxLength={6} placeholder={t.code} value={room} onChange={e=>setRoom(e.target.value.toUpperCase())}/><button onClick={joinRoom}>{t.join}</button></div>:<><div className="room-code">{botBattle?'🤖 '+(selectedBot?.name||'Rival ficticio'):roomCode}</div>{!botBattle&&<button className="copy-btn" onClick={()=>navigator.clipboard?.writeText(roomCode)}>📋 Copiar código</button>}<div className="status">{roomStatus}</div><div className="battle-actions"><button onClick={startCamera}>📷 {cameraOn?'CÁMARA ACTIVA':'ACTIVAR CÁMARA'}</button>{botBattle&&<button type="button" onClick={clearBotOpponent}>← Cambiar rival</button>}<button className="primary" disabled={!opponentJoined||!cameraOn||!bothCamerasReady||(!botBattle&&!poseReady)||battleStarted} onClick={startBattle}>🔥 {battleStarted?'BATALLA EN CURSO':(!poseReady&&!botBattle)?'CARGANDO IA…':battleReady?'⏳ LISTO — ESPERANDO RIVAL':opponentReady?'⚔️ RIVAL LISTO · YO TAMBIÉN':'✓ ESTOY LISTO'}</button>{battleStarted&&<button onClick={finishBattle}>🏁 Terminar y guardar resultado</button>}</div></>}</div><div className="video-grid"><VideoCard title={user.displayName||'Jugador 1'} videoRef={videoRef} score={aura} muted/><VideoCard title={botBattle?`${selectedBot?.emoji||'🤖'} ${selectedBot?.name||'Rival ficticio'}`:'Rival'} videoRef={remoteVideoRef} score={opponentAura} bot={botBattle}/></div><div className="battle-timers">{battleCountdown>0&&<div className="battle-timer prep-timer"><small>⚔️ INICIO DE BATALLA · COMIENZA EN</small><strong className="countdown-number">{battleCountdown}</strong><span>🔥 ¡Prepárate! La batalla comienza en {battleCountdown} segundo{battleCountdown===1?'':'s'}</span></div>}{battleStarted&&<div className="battle-timer live-timer"><small>⚔️ BATALLA · TIEMPO RESTANTE</small><strong>{battleSeconds}</strong><span>🔥 ¡Gana Aura antes de que llegue a 0!</span></div>}</div>{battleStarted&&<div className="battle-banner"><div>🧠 IA ANALIZANDO · ⏱️ {battleSeconds}s · ⚡ {aura} Aura</div><div className="move-detection">{detectedMove}{moveBonus>0&&<span> +{moveBonus}</span>}</div> <button className="music-toggle" onClick={()=>setMusicOn(v=>!v)}>{musicOn?'🔊 Música':'🔇 Música'}</button></div>}</>}</Panel>}
 {tab==='ai'&&<Panel title="🤖 IA Aura"><div className="training-notice"><strong>🎯 ÁREA DE ENTRENAMIENTO</strong><span>Entrena durante 15 segundos con la IA de movimiento y mejora tu control.</span><small>🏆 Cada entrenamiento completado otorga <b>+1 Aura</b>.</small></div><div className="ai-camera-card"><div className="ai-camera-frame"><video ref={aiVideoRef} autoPlay playsInline muted/>{!cameraOn&&<div className="ai-camera-placeholder">📷<span>Activa tu cámara para entrenar y medir tu Aura</span></div>}<div className="ai-camera-badge">{cameraOn?'● CÁMARA ACTIVA':'● CÁMARA INACTIVA'}</div>{cameraOn&&<div className="ai-aura-overlay"><span>⚡ AURA</span><b>{aura}</b></div>}</div><div className="ai-aura-label">⚡ Aura detectada: <strong>{aura}</strong></div><div className="ai-meter"><div className="meter-fill" style={{width:`${aura}%`}}/></div></div><div className="ai-hero"><div><h2>{trainingActive?'Entrenamiento en curso':'Tu Aura de entrenamiento'}</h2><div className="aura-number">{trainingActive?trainingSeconds:aura}</div><p>{trainingActive?`Mantén tus movimientos durante ${trainingSeconds} segundos.`:'La IA analiza tus movimientos corporales localmente.'}</p></div><div className="ai-orb">⚡</div></div><div className="ai-training-actions"><button type="button" className="ai-menu-card" onClick={startCamera} disabled={trainingActive}><span>📷</span><strong>{cameraOn?'Cámara conectada':'Activar cámara'}</strong><small>{cameraOn?'La cámara está lista para analizar tus movimientos.':'Activa tu cámara para comenzar el entrenamiento.'}</small></button><button type="button" className="ai-menu-card" onClick={()=>{if(!cameraOn){setRoomStatus('📷 Activa primero la cámara.');return}if(!poseLandmarkerRef.current){setRoomStatus('🧠 La IA todavía está cargando. Espera unos segundos.');return}movementScoreRef.current=0;previousPoseRef.current=null;poseHistoryRef.current=[];poseFrameCountRef.current=0;poseVisibleFrameCountRef.current=0;setAura(0);setTrainingDone(false);setTrainingSeconds(15);setTrainingActive(true);setRoomStatus('🧠 Entrenamiento iniciado. ¡15 segundos!')}} disabled={!cameraOn||trainingActive||!poseReady}><span>🔥</span><strong>{trainingActive?`Entrenamiento · ${trainingSeconds}s`:'Iniciar entrenamiento'}</strong><small>{trainingActive?'Entrenamiento en curso. Mantén tus movimientos.':'Entrena durante 15 segundos y gana +1 Aura.'}</small></button></div>{trainingDone&&<div className="notice">🎉 ¡Entrenamiento completado! <strong>+1 Aura</strong> se añadió a tu perfil.</div>}<div className="notice">Consejo: buena iluminación, cuerpo visible y encuadre estable ayudan a obtener una señal más consistente.</div></Panel>}
 ...
 {tab==='clans'&&<Panel title="🛡️ Clanes">
   {clanMenu!=='menu'&&<button type="button" className="clan-back-btn" onClick={()=>{setClanMenu('menu');setSelectedClan(null);setClanMsg('')}}>← Menú de clanes</button>}
   {clanMenu==='menu'&&<div className="clan-menu-grid">
     <button type="button" className="clan-menu-card" onClick={()=>setClanMenu('mine')}><span>🏰</span><strong>Mi clan</strong><small>Ver tu clan, miembros y chat</small></button>
     <button type="button" className="clan-menu-card" onClick={()=>setClanMenu('search')}><span>🔎</span><strong>Buscar clan</strong><small>Busca por nombre o etiqueta</small></button>
     <button type="button" className="clan-menu-card" onClick={()=>setClanMenu('create')}><span>👑</span><strong>Crear clan</strong><small>Crea tu propio clan</small></button>
     <button type="button" className="clan-menu-card" onClick={()=>setClanMenu('list')}><span>📋</span><strong>Lista de clanes</strong><small>Explora los clanes disponibles</small></button>
   </div>}
   {clanMenu==='mine'&&!myClanId&&<div className="empty">No perteneces a ningún clan todavía. Puedes crear uno o buscar un clan.</div>}
   {clanMenu==='mine'&&myClanId&&!selectedClan&&<div className="notice">🏰 Cargando tu clan…</div>}
   {clanMenu==='create'&&<div className="clan-create">
   ...
   {(clanMenu==='search'||clanMenu==='list')&&<div className="clan-directory">   <div className="section-title">🔎 Buscar clanes</div>
   ...
   <div className="clan-list">
     {filteredClans.length ? filteredClans.map(c=>(
       <div className="clan-card clan-card-v163" key={c.id}>
         ...
         <button type="button" className="secondary clan-view-btn" onClick={()=>{setSelectedClan(c);setClanMenu('mine');setClanMsg('')}}>Ver clan</button>
       </div>
     )) : <div className="empty">No encontramos clanes con esa búsqueda.</div>}
   </div>

</div>}
   {selectedClan&&<div className="clan-detail-wrap"><div className="clan-detail">
  ...
  return (
    <div className="auth-shell">
      <div className="auth-brand">
        <div className="brand">⚡ <span>Aura farming battles</span><b>V5.176.8</b></div>
...
function VideoCard({title,videoRef,score,muted=false,bot=false}:{title:string;videoRef:RefObject<HTMLVideoElement|null>;score:number;muted?:boolean;bot?:boolean}){
 const [rotatePortrait,setRotatePortrait]=useState(false)
 useEffect(()=>{
  if(bot)return
  const video=videoRef.current
  if(!video)return
  const update=()=>setRotatePortrait(video.videoWidth>video.videoHeight)
  video.addEventListener('loadedmetadata',update)
  video.addEventListener('resize',update)
  update()
  return()=>{video.removeEventListener('loadedmetadata',update);video.removeEventListener('resize',update)}
 },[videoRef,bot])
 return <div className="video-card"><h3><span>{title}</span>{bot&&<small className="bot-badge">BOT</small>}</h3><div className="video-frame vertical-camera-frame">{bot?<div className="bot-camera-placeholder"><div className="bot-live-avatar"><span>🤖</span></div><strong>Rival IA</strong><small>Jugador ficticio · batalla activa</small><b>⚡ {score} Aura</b></div>:<video ref={videoRef} className={rotatePortrait?'camera-rotated':''} autoPlay playsInline muted={muted}/>}</div><div className="score-line"><div className="aura-label"><span>⚡ AURA</span><b>{score}</b></div><div className="bar"><span style={{width:`${Math.min(100,score)}%`}}/></div></div></div>}

...
import { addDoc, collection, deleteDoc, doc, getDoc, getDocs, limit, onSnapshot, orderBy, query, serverTimestamp, setDoc, updateDoc, arrayUnion, where, writeBatch, increment, runTransaction } from 'firebase/firestore'
...
export default function App(){
 ...
 const [leaders,setLeaders]=useState<Friend[]>([]); const [clans,setClans]=useState<Clan[]>([]); const [clanSearch,setClanSearch]=useState(''); const [clanName,setClanName]=useState(''); const [clanTag,setClanTag]=useState(''); const [clanDescription,setClanDescription]=useState(''); const [clanVisibility,setClanVisibility]=useState<'public'|'private'>('public'); const [myClanId,setMyClanId]=useState<string|null>(null); const [clanMsg,setClanMsg]=useState('')
...
useEffect(()=>{
 ...
 async function createClan(){
  ...
  try{
    ...
    batch.set(clanRef,{name,tag,description,visibility:clanVisibility,owner:user.uid,members:[user.uid],createdAt:serverTimestamp()});

...
type Clan = {id:string; name:string; owner:string; members:string[]; tag?:string; description?:string; visibility?:'public'|'private'; createdAt?:any}
type ClanJoinRequest = {id:string; clanId:string; requesterId:string; requesterName:string; clanName?:string; status:'pending'|'accepted'|'rejected'; createdAt?:any}
...
useEffect(()=>{
 ...
 async function requestToJoinClan(clan:Clan){
  ...
  try{const snap=await getDoc(ref); if(snap.exists()&&snap.data().status==='pending'){setClanMsg('ℹ️ Ya tienes una solicitud pendiente para este clan.');return}
 ...
 async function approveClanRequest(r:ClanJoinRequest){
  ...
  try{await runTransaction(db,async tx=>{const clanRef=doc(db,'clans',selectedClan.id);const reqRef=doc(db,'clanJoinRequests',r.id);const clanSnap=await tx.get(clanRef);const reqSnap=await tx.get(reqRef);if(!clanSnap.exists()||!reqSnap.exists())throw new Error('request-missing');const clan=clanSnap.data();const req=reqSnap.data();if(clan.owner!==user.uid||req.status!=='pending')throw new Error('not-allowed');const members=Array.isArray(clan.members)?clan.members:[];if(!members.includes(req.requesterId))members.push(req.requesterId);tx.update(clanRef,{members});tx.update(reqRef,{status:'accepted',reviewedAt:serverTimestamp()})});setClanMsg(`✅ ${r.requesterName||'Jugador'} se unió al clan.`)}catch(error:any){console.error('Approve clan request error:',error);setClanMsg(`❌ No se pudo aprobar la solicitud${error?.message?` (${error.message})`:error?.code?` (${error.code})`:''}.`)}
 }
 ...
 async function leaveClan(clan:Clan){
  if(!user)return
  if(clan.owner===user.uid){setClanMsg('👑 El Líder no puede salir del clan todavía. En v5.164 podrá transferir el liderazgo.');return}
  if(!clan.members?.includes(user.uid))return
  try{await updateDoc(doc(db,'clans',clan.id),{members:arrayRemove(user.uid)});setSelectedClan(null);setClanMsg(`🚪 Has salido de [${clan.tag||'CLAN'}] ${clan.name}.`)}catch(error:any){console.error('Leave clan error:',error);setClanMsg(`❌ No se pudo salir del clan${error?.code?` (${error.code})`:''}.`)}
 }
 const pendingRequestForSelected=selectedClan?myClanRequests.find(r=>r.clanId===selectedClan.id&&r.status==='pending'):undefined
  ...
  return (
    <div className="auth-shell">
      <div className="auth-brand">
        <div className="brand">⚡ <span>AURA BATTLE</span><b>V5.163</b></div>

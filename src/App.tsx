 ...
 async function inviteFriendToClan(clan:Clan, friend:Friend){
  ...
  try{
    ...
    if(snap.exists()){
      ...
      if(data.status==='pending'&&data.kind==='invite'){
        setClanMsg(`ℹ️ Ya enviaste una invitación a ${friend.name}.`)
        return
      }
      if(data.status==='accepted'){
        ...
        if(members.includes(friend.id)){
          setClanMsg(`ℹ️ ${friend.name} ya pertenece al clan.`)
          return
        }
      }
      await deleteDoc(ref)

 ...
 async function inviteFriendToClan(clan:Clan, friend:Friend){
  ...
  try{
    ...
    if(snap.exists()){
      ...
      if(data.status==='pending'&&data.kind==='invite'){setClanMsg(`ℹ️ Ya enviaste una invitación a ${friend.name}.`);return}
      if(data.status==='accepted'){
        const freshClan=await getDoc(doc(db,'clans',clan.id))
        const members=freshClan.exists()&&Array.isArray(freshClan.data().members)?freshClan.data().members:[]
        if(members.includes(friend.id)){setClanMsg(`ℹ️ ${friend.name} ya pertenece al clan.`);return}
      }
      await deleteDoc(ref)
    }
 ...
 async function acceptClanInvite(r:ClanJoinRequest){
  ...
  try{
    ...
    await runTransaction(db,async tx=>{
      ...
      if(req.receiverId!==user.uid||req.kind!=='invite'||req.status!=='pending')throw new Error('not-allowed')
    ...
    setUnreadClanInvites(prev=>prev.filter(x=>x.id!==r.id)); localStorage.setItem(`auraClanInviteRead:${user.uid}:${r.id}`,'1'); setClanMsg(`✅ Te uniste a [${String(clan.tag||'CLAN')}] ${String(clan.name||'Clan')}.`)

...
useEffect(()=>{
 ...
 function selectBotOpponent(bot:BotOpponent){
   void resetPeer()
   setSelectedBot(bot);setBotBattle(true);setRoomCode(`BOT-${bot.id.slice(-2)}`);setRoom('');setHost(true);hostRef.current=true;setOpponentJoined(true);setBothCamerasReady(true);setOpponentReady(true);setBattleReady(false);setBattleResult(null);setOpponentAura(0);setRoomStatus(`🤖 ${bot.name} está disponible. Activa tu cámara y pulsa LISTO.`);navigateTab('battle')
 }
  ...
  return (
    <div className="auth-shell">
      <div className="auth-brand">
        <div className="brand">⚡ <span>Aura farming battles</span><b>V5.176.5</b></div>
...
function VideoCard({title,videoRef,score,muted=false,bot=false}:{title:string;videoRef:RefObject<HTMLVideoElement|null>;score:number;muted?:boolean;bot?:boolean}){return <div className="video-card"><h3><span>{title}</span>{bot&&<small className="bot-badge">BOT</small>}</h3><div className="video-frame vertical-camera-frame">{bot?<div className="bot-camera-placeholder"><span>🤖</span><strong>Rival IA</strong><small>Jugador ficticio</small><b>⚡ {score} Aura</b></div>:<video ref={videoRef} autoPlay playsInline muted={muted}/>}</div><div className="score-line"><div className="aura-label"><span>⚡ AURA</span><b>{score}</b></div><div className="bar"><span style={{width:`${Math.min(100,score)}%`}}/></div></div></div>}
